import type { BaseClientOptions, BaseRequestOptions } from "../../../../../../../../BaseClient.mjs";
import { type NormalizedClientOptionsWithAuth } from "../../../../../../../../BaseClient.mjs";
import * as core from "../../../../../../../../core/index.mjs";
import * as Deepgram from "../../../../../../../index.mjs";
import { BillingClient } from "../resources/billing/client/Client.mjs";
import { KeysClient } from "../resources/keys/client/Client.mjs";
import { MembersClient } from "../resources/members/client/Client.mjs";
import { ModelsClient } from "../resources/models/client/Client.mjs";
import { RequestsClient } from "../resources/requests/client/Client.mjs";
import { UsageClient } from "../resources/usage/client/Client.mjs";
export declare namespace ProjectsClient {
    type Options = BaseClientOptions;
    interface RequestOptions extends BaseRequestOptions {
    }
}
export declare class ProjectsClient {
    protected readonly _options: NormalizedClientOptionsWithAuth<ProjectsClient.Options>;
    protected _keys: KeysClient | undefined;
    protected _members: MembersClient | undefined;
    protected _models: ModelsClient | undefined;
    protected _requests: RequestsClient | undefined;
    protected _usage: UsageClient | undefined;
    protected _billing: BillingClient | undefined;
    constructor(options?: ProjectsClient.Options);
    get keys(): KeysClient;
    get members(): MembersClient;
    get models(): ModelsClient;
    get requests(): RequestsClient;
    get usage(): UsageClient;
    get billing(): BillingClient;
    /**
     * Retrieves basic information about the projects associated with the API key
     *
     * @param {ProjectsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Deepgram.BadRequestError}
     *
     * @example
     *     await client.manage.v1.projects.list()
     */
    list(requestOptions?: ProjectsClient.RequestOptions): core.HttpResponsePromise<Deepgram.ListProjectsV1Response>;
    private __list;
    /**
     * Retrieves information about the specified project
     *
     * @param {string} project_id - The unique identifier of the project
     * @param {Deepgram.manage.v1.ProjectsGetRequest} request
     * @param {ProjectsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Deepgram.BadRequestError}
     *
     * @example
     *     await client.manage.v1.projects.get("123456-7890-1234-5678-901234", {
     *         limit: 1.1,
     *         page: 1.1
     *     })
     */
    get(project_id: string, request?: Deepgram.manage.v1.ProjectsGetRequest, requestOptions?: ProjectsClient.RequestOptions): core.HttpResponsePromise<Deepgram.GetProjectV1Response>;
    private __get;
    /**
     * Deletes the specified project
     *
     * @param {string} project_id - The unique identifier of the project
     * @param {ProjectsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Deepgram.BadRequestError}
     *
     * @example
     *     await client.manage.v1.projects.delete("123456-7890-1234-5678-901234")
     */
    delete(project_id: string, requestOptions?: ProjectsClient.RequestOptions): core.HttpResponsePromise<Deepgram.DeleteProjectV1Response>;
    private __delete;
    /**
     * Updates the name or other properties of an existing project
     *
     * @param {string} project_id - The unique identifier of the project
     * @param {Deepgram.manage.v1.UpdateProjectV1Request} request
     * @param {ProjectsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Deepgram.BadRequestError}
     *
     * @example
     *     await client.manage.v1.projects.update("123456-7890-1234-5678-901234")
     */
    update(project_id: string, request?: Deepgram.manage.v1.UpdateProjectV1Request, requestOptions?: ProjectsClient.RequestOptions): core.HttpResponsePromise<Deepgram.UpdateProjectV1Response>;
    private __update;
    /**
     * Removes the authenticated account from the specific project
     *
     * @param {string} project_id - The unique identifier of the project
     * @param {ProjectsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Deepgram.BadRequestError}
     *
     * @example
     *     await client.manage.v1.projects.leave("123456-7890-1234-5678-901234")
     */
    leave(project_id: string, requestOptions?: ProjectsClient.RequestOptions): core.HttpResponsePromise<Deepgram.LeaveProjectV1Response>;
    private __leave;
}
